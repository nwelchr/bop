...
export const RECEIVE_PLAYLIST_WITH_TRACKLIST =
...
export const fetchPlaylistThenPlaySong = (playlistId, params) => (dispatch) =>
  APIUtil.fetchPlaylist(playlistId).then((payload) => {
    ...
    const songToPlay =
      payload.songs[Object.values(payload.playlist.songIds)[0]];
    // true is for playing first song in the tracklist/shuffledTracklist
    dispatch(playSong(songToPlay, params, true));
  });

export const createPlaylist = (playlist) => (dispatch) =>
  APIUtil.createPlaylist(playlist).then((receivedPlaylist) =>
    dispatch(receiveCreatedPlaylist(receivedPlaylist))
  );

export const updatePlaylist = (playlist) => (dispatch) =>
  APIUtil.updatePlaylist(playlist).then((returnedPlaylist) =>
