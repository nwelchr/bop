...
class SongIndexItem extends React.Component {
    ...
    if (this.props.currentSong) {
      if (this.props.playing) {
        if (
          ...
          Object.values(this.props.match.params)[0] ===
  ...
  playSongWithTracklist() {
    ...
    if (this.props.match.path === '/collection/songs') {
      ...
      const tracklistParam = tracklistParamKey.slice(
        0,
        tracklistParamKey.length - 2
      );
      tracklist = this.props[tracklistParam].songIds;
    }
    this.props.playSongWithTracklist(
      this.props.song,
      this.props.match.params || { collection: 'song' },
      tracklist
    );
  }
  ...
  openAddToPlaylistModal(e) {
    e.stopPropagation();
    this.props
      .fetchModalPlaylists(this.props.song.id)
      .then((response) => this.props.openAddToPlaylistForm());
  }

  removeFromPlaylist(e) {
    e.stopPropagation();
    this.props
      .removeSongFromPlaylist(this.props.song.id, this.props.playlist.id)
      .then((response) => this.props.fetchPlaylist(this.props.playlist.id));
  }
