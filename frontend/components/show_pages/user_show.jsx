...
class UserShow extends React.Component {
  ...
  handleClick(followText) {
    ...
    followText === 'Follow'
      ? follow(user.id).then(this.setState({ followText: 'Unfollow' }))
      : unfollow(user.id).then(this.setState({ followText: 'Follow' }));
  }
  ...
  followingResults() {
    ...
    const followedUsers =
      this.props.user.followedUsers.length > 0 ? (
        <section className="followed-users">
  ...
  render() {
    if (
      ...
      typeof this.props.user.playlists === 'undefined'
    ) {
      return <div />;
    } else {
      ...
      if (currentUser.id !== user.id) {
        let followText =
          currentUser.followed_users &&
          currentUser.followed_users.includes(user.id)
            ? 'Unfollow'
            : 'Follow';
        followButton = (
          <button
      ...
      if (user.playlists && user.playlists.length > 0)
        playlistHeading = <h1 className="">Public Playlists</h1>;

      let numFollowers =
        user.followerIds !== null
                ...
                <div className="music-index">
                  <div className="music-index-wrapper">
                    ...
                    <Route
                      ...
                      render={() => <UserResults users={user.followers} />}
                    />
                    <Route
